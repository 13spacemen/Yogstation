...
/mob/living/carbon/human/update_inv_w_uniform()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		if(dna && dna.species.sexes && dna.species.is_dimorphic)
			var/G = (gender == FEMALE) ? "f" : "m"
			if(G == "f" && U.fitted != NO_FEMALE_UNIFORM)
				uniform_overlay = U.build_worn_icon(default_layer = UNIFORM_LAYER, default_icon_file = DEFAULT_UNIFORM_FILE, isinhands = FALSE, femaleuniform = U.fitted, override_state = target_overlay)
		...
		if(!uniform_overlay)
			...
			var/icon_to_use = DEFAULT_UNIFORM_FILE
			if(l_leg?.species_id == r_leg?.species_id == SPECIES_VOX)//for Vox, it's the Vox legs that make regular sprites not fit
				if(icon_exists(U.sprite_sheets[l_leg.species_id], U.icon_state))
					icon_to_use = U.sprite_sheets[l_leg.species_id]
					U.species_fitted = l_leg.species_id
				else
					U.species_fitted = null
			else if(chest?.species_id in U.sprite_sheets)
				if(icon_exists(U.sprite_sheets[chest.species_id], U.icon_state))
					icon_to_use = U.sprite_sheets[chest.species_id]
					U.species_fitted = chest.species_id
				else
					U.species_fitted = null
			uniform_overlay = U.build_worn_icon(default_layer = UNIFORM_LAYER, default_icon_file = icon_to_use, isinhands = FALSE, override_state = target_overlay)
